"""Mock Laminus sidecar for E2E testing.

Serves canned profile catalog and completes slice jobs synchronously.
Run via docker-compose.test.yml, or standalone:
    uvicorn laminus_mock:app --host 0.0.0.0 --port 5000
"""
...
# Stable UUIDs — used by tests and by the mock's own /api/test/known-profile
...
_MOCK_GCODE = b"; mock gcode from laminus_mock\nG28\nM109 S220\nG1 X128 Y128 Z0.2 F3000\nM104 S0\nM84\n"

_jobs: dict[str, str] = {}  # job_id → status ("completed")

app = FastAPI(title="Laminus Mock", version="mock")
...
async def pack_stls(request: Request):
    # Accept any combination of files + form data; always return minimal 3MF
...
@app.get("/api/test/known-profile")
async def test_known_profile():
    """Return the mock's stable profile UUIDs for E2E test setup."""
    return {
